...
const GameBoard: React.FC<Props> = ({ gameState, onExplode, direction }) => {
  ...
  const shakeIntensity = gameState === 'PLAYING' ? Math.max(0, (15 - timeLeft) / 3) : 0;

  return (
    <div className="relative w-64 h-64 flex items-center justify-center">
      {/* Vòng xoay hướng */}
      {gameState === 'PLAYING' && (
        <div className={`absolute inset-0 border-4 border-dashed border-orange-500/20 rounded-full ${direction === 'CW' ? 'animate-spin-slow' : 'animate-spin-reverse-slow'}`}></div>
      )}
